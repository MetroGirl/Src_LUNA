...
ConsumeStructuredBuffer<uint> gConsumeMoon : register(u2);
Buffer<uint> gMoonRestCount : register(t0);
...
[numthreads(256, 1, 1)]
void EmitMoonCS(uint id : SV_DispatchThreadID)
{
	if (cEmitCount <= id || gMoonRestCount[6] <= id) return;
	...
	uint newIndex = gConsumeMoon.Consume();
